...
func (m Migrator) CreateSequence(tx *gorm.DB, stmt *gorm.Statement, field *schema.Field,
	...
	if err = tx.Exec(`CREATE SEQUENCE ? AS ?`, clause.Expr{SQL: sequenceName},
...
func (m Migrator) UpdateSequence(tx *gorm.DB, stmt *gorm.Statement, field *schema.Field,
	...
	if err = tx.Exec(`ALTER SEQUENCE ? AS ?`, clause.Expr{SQL: sequenceName}, clause.Expr{SQL: serialDatabaseType}).Error; err != nil {
		return err
	}
...
func (m Migrator) DeleteSequence(tx *gorm.DB, stmt *gorm.Statement, field *schema.Field,
	...
	if err = tx.Exec(`DROP SEQUENCE ?`, clause.Expr{SQL: sequenceName}).Error; err != nil {
		return err
	}
